...
def createObstaclesParameterized(obsPctCvrg, n, m):
    s = {}
    for i in range(n*m):
        s[i] = {i}
    ...
    used = []
    for c in range(n*m):
        if c == 0 or c == (n - 1) or c == (n * m - m) or c == (n * m - 1): continue
        e.append({c, c - 1})
        e.append({c, c + 1})
        e.append({c, c - n})
        e.append({c, c + n})
    ...
    while len(s) > 1:
        ...
        if s[edge[0]] == s[edge[1]]:
            s[edge[0]] == s[edge[0]].union(s[edge[1]])
            s.pop(edge[1])
...
obstacles = createObstaclesParameterized(obsPctCvrg, n, m)
# print(obstacles)
