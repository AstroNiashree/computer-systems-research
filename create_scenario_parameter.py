...
def createObstaclesParameterized(n, m):
    s = {}
    for i in range(n*m):
        s[i] = ({i})
    e = {}
    ...
    used = []
    while len(s) > 1:
        edgeNum = random.choice(list(e.keys()))
        while edgeNum in used:
            edgeNum = random.choice(list(e.keys()))
        edge = e[edgeNum]
        edgeNumNum = -1
        if len(edge) == 2:
            edgeNumNum = random.randint(0, 1)
            edge = e[edgeNum][edgeNumNum]
        else:
            edge = e[edgeNum][0]
        x, y = edge[0], edge[1]
        u, v = find(s, x), find(s, y)

        if u != v:
            s[u] = s[u].union(s[v])
            s.pop(v)
            if edgeNumNum != -1:
                e[edgeNum].pop(edgeNumNum)
            else:
                e.pop(edgeNum) #
        else:
            used.append(edgeNum)
...
obstacles = createObstaclesParameterized(n, m)
print(obstacles)
